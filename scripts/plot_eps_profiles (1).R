...
# plot all eps values over depth
plot_eps_all <- ggplot(data = filter(datj_lat, eps %in% c('eps1','eps2')), 
                                  mapping = aes(x = log10(eps_val), y = pressure, col = eps)) +
  geom_point(alpha = 0.3) +
  scale_y_reverse(limits=c(500,0)) +
  scale_x_continuous(breaks = seq(-9, -6, by = 1), limits = c(-9.3, -5.7),
                     label = c(expression(10^-9), expression(10^-8), expression(10^-7), expression(10^-6))) + 
  labs(x = 'eps (m^3/s^2)',
       y = 'depth (m)',
       title = 'all eps profiles',
       subtitle = 'colours indicate shear probe 1 or 2') +
  theme_bw()

# write plot to file
#pdf('figures/eps_all-profiles.pdf', width=7/sqrt(2),height=7)
#plot_eps_all
#dev.off()
